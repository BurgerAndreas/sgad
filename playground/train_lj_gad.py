...
# ---------------------------------------------
def lennard_jones_energy(positions: torch.Tensor, epsilon: float = 1.0, sigma: float = 1.0) -> torch.Tensor:
    """
...
# ---------------------------------------------
def compute_forces_hessian_autograd(positions: torch.Tensor, epsilon: float = 1.0, sigma: float = 1.0):
    """
...
# ---------------------------------------------
def lennard_jones_energy_vectorized(positions: torch.Tensor, epsilon: float = 1.0, sigma: float = 1.0) -> torch.Tensor:
    """
    ...
    mask = torch.triu(torch.ones(N, N, device=device, dtype=torch.bool), diagonal=1)  # (N, N)
...
def compute_forces_hessian_vectorized(positions: torch.Tensor, epsilon: float = 1.0, sigma: float = 1.0):
    """
    ...
    # Identity matrix
    identity = torch.eye(3, device=device, dtype=dtype).view(1, 1, 1, 3, 3)  # (1, 1, 1, 3, 3)
...
def verify_vectorized_implementations(num_samples: int = 5) -> None:
    ...
    energy_rel_diff = ((energy_autograd - energy_vec).abs() / (energy_autograd.abs() + 1e-8)).max().item()
    ...
    force_rel_diff = ((forces_autograd - forces_vec).abs() / (forces_autograd.abs() + 1e-8)).max().item()
    hessian_rel_diff = ((hessian_autograd - hessian_vec).abs() / (hessian_autograd.abs() + 1e-8)).max().item()
...
def main(
    ...
    def lr_schedule(iteration):
        return (final_lr + (initial_lr - final_lr) * (1 - iteration / num_iterations)) / initial_lr
    ...
    if load_checkpoint and os.path.isfile(ckpt_path):
        ...
        for iter_idx in tqdm(range(num_iterations)):
            ...
            if iter_idx % log_every == 0 and iter_idx > 0:
                ...
                tqdm.write(f"Iteration {iter_idx}: loss={loss.item():.4f}, avg={avg_loss:.4f}")
    ...
    with torch.no_grad():
        ...
        n_steps = 1000
        n_samples = 16
        x_det = torch.zeros((n_samples, n_steps + 1, D), dtype=torch.float32, device=device)
        
        # Random initial coordinates
        x_det[:, 0, :] = 2.0 * pos_range * (torch.rand((n_samples, D), device=device) - 0.5)
    ...
    with torch.no_grad():
        ...
        n_steps_s = 1000
        eta = 0.3
        x_stoch = torch.zeros((n_samples, n_steps_s + 1, D), dtype=torch.float32, device=device)
        x_stoch[:, 0, :] = 2.0 * pos_range * (torch.rand((n_samples, D), device=device) - 0.5)
        t_dummy = torch.zeros(n_samples, device=device)
    ...
    print(f"  Initial energy (avg): {energies_det[:, 0].mean():.4f} ± {energies_det[:, 0].std():.4f}")
    print(f"  Final energy (avg):   {energies_det[:, -1].mean():.4f} ± {energies_det[:, -1].std():.4f}")
    print(f"  Final RMS:            {rms_det[-1]:.4f}")
    print("\nStochastic sampling:")
    print(f"  Initial energy (avg): {energies_stoch[:, 0].mean():.4f} ± {energies_stoch[:, 0].std():.4f}")
    print(f"  Final energy (avg):   {energies_stoch[:, -1].mean():.4f} ± {energies_stoch[:, -1].std():.4f}")
    ...
    for i in range(min(8, n_samples)):
        pos = final_det[i]
        ax1.scatter(pos[:, 0], pos[:, 1], pos[:, 2], s=100, alpha=0.6)
    ax1.set_xlabel('X')
    ax1.set_ylabel('Y')
    ax1.set_zlabel('Z')
    ax1.set_title(f'Deterministic (COM removed)\nAvg dist: {avg_dist_det.mean():.2f}±{avg_dist_det.std():.2f}')
    
    # Plot stochastic samples
    ax2 = fig.add_subplot(1, 2, 2, projection='3d')
    for i in range(min(8, n_samples)):
        pos = final_stoch[i]
        ax2.scatter(pos[:, 0], pos[:, 1], pos[:, 2], s=100, alpha=0.6)
    ax2.set_xlabel('X')
    ax2.set_ylabel('Y')
    ax2.set_zlabel('Z')
    ax2.set_title(f'Stochastic (COM removed)\nAvg dist: {avg_dist_stoch.mean():.2f}±{avg_dist_stoch.std():.2f}')
...
uv run playground/train_lj_gad.py --score-clip 1e20;
uv run playground/train_lj_gad.py --final-lr 1e-6;
uv run playground/train_lj_gad.py --num-iterations 1e6;
uv run playground/train_lj_gad.py --loss-type l1;
echo "Done"
"""
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train GAD field on Lennard-Jones potential")
    parser.add_argument("--use-egnn", type=bool, default=True, help="Use EGNN instead of MLP")
    parser.add_argument("--score-clip", type=float, default=1e6, help="Clipping value for scores")
    parser.add_argument("--initial-lr", type=float, default=1e-3, help="Initial learning rate")
    parser.add_argument("--final-lr", type=float, default=5e-5, help="Final learning rate")
    parser.add_argument("--batch-size", type=int, default=128, help="Batch size")
    parser.add_argument("--pos-range", type=float, default=3.0, help="Range of positions")
    parser.add_argument("--num-iterations", type=int, default=500_000, help="Number of iterations")
    parser.add_argument("--loss-type", type=str, default="l2", choices=["l1", "l2"], help="Loss function type")
    parser.add_argument("--load-checkpoint", action="store_true", help="Load existing checkpoint")
    parser.add_argument("--no-verify", action="store_true", help="Skip derivative verification")
