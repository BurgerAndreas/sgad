...
class ScoreNet(nn.Module):
    ...
    def __init__(self, dim, hidden=512, k=8):
        ...
        self.net = nn.Sequential(
            nn.Linear(dim + 2*k, hidden), nn.SiLU(),
    ...
    def sigma_embed(self, sigma):  # sigma: (B,)
        ...
        s = torch.sin(freqs * logsig);  c = torch.cos(freqs * logsig)
...
def train_on_gad(
    ...
    batch_size=32, epochs=50, lr=2e-4,
    sigma_min=1e-3, sigma_max=1e-1, num_sigmas=10,
...
@torch.no_grad()
def annealed_langevin_sampling(model, sigmas, N, num_samples=64, steps_per_sigma=50, step_size=1e-2, device=None):
    """
    Not a valid reverse-SDE unless the field is a true score. It is a heuristic generator.
    """
    device = device or next(model.parameters()).device
    D = 3 * N
    x = torch.randn(num_samples, D, device=device) * sigmas[0]      # init from wide Gaussian
