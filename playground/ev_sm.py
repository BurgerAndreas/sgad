...
def score_star(x, energy_fn, mu=0.1):
    ...
    Hg = torch.autograd.grad(g, x, grad_outputs=g.view_as(x), create_graph=True)[
        0
    ].reshape(D)
    ...
    Hv1 = torch.autograd.grad(g, x, grad_outputs=v1.view_as(x), create_graph=True)[
        0
    ].reshape(D)
    q1 = (Hv1 * v1).sum()
    grad_l1 = torch.autograd.grad(q1, x, create_graph=True)[0].reshape(D)

    Hv2 = torch.autograd.grad(g, x, grad_outputs=v2.view_as(x), create_graph=True)[
        0
    ].reshape(D)
    q2 = (Hv2 * v2).sum()
...
def train_on_field(
    energy_fn, N, steps=2000, batch_size=8, lr=2e-4, mu=0.1, init_std=1.0, device="cpu"
):
    torch.manual_seed(0)
    D = 3 * N
    ...
    for t in range(steps):
        ...
        with torch.no_grad():
            S = torch.stack(
                [
                    score_star(x[b], energy_fn, mu).reshape(-1)
...
@torch.no_grad()
def langevin_sample(
    model, n_samples, N, steps=2000, step_size=1e-3, noise=1.0, device="cpu"
):
    D = 3 * N
