...
from sgad.utils.data_utils import graph_to_vector, subtract_com_batch
...
def adjoint_score_target(
    ...
    #     grad_E["reg_grad"].to(graph_state_1["pos"].device)
    # )
...
def SOC_loss(controls, graph_state, energies, noise_schedule):
    ...
    x1 = graph_to_vector(graph_state["pos"], n_particles, n_spatial_dim)
    ...
    n = x1.shape[-1]
    sigma_1 = noise_schedule.h(torch.Tensor([1.0]).to(graph_state["pos"].device))
    log_pb_1 = (
